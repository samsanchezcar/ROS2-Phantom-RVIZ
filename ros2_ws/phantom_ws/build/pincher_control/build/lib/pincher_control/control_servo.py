    ...
    def initialize_motors(self, goal_positions, moving_speed, torque_limit):
        ...
        for dxl_id, goal in zip(self.dxl_ids, goal_positions):
            ...
            try:
                ...
                self.current_joint_positions[joint_index] = self.dxl_to_radians(goal)
    ...
    def move_motor(self, motor_id, position):
        ...
        try:
            ...
            if result == 0:
                ...
                self.current_joint_positions[joint_index] = self.dxl_to_radians(position)
...
• Feedback visual de los movimientos
    ...
    def launch_rviz(self):
        ...
        try:
            ...
            # Comando para lanzar RViz con configuración para PhantomX Pincher

            cmd = ["rviz2", "-d", self.get_rviz_config_path()]
            ...
            def run_rviz():
                ...
                # Cuando RViz se cierra, actualizar la interfaz

                self.window.after(0, self.on_rviz_closed)

            

            thread = threading.Thread(target=run_rviz)
            ...
            self.status_label.config(text="RViz lanzado - Visualizando robot")
